...
cleanup_vpc_resources() {
  ...
  for sg_id in ${sg_ids}; do
    ...
    aws --region "${REGION}" ec2 revoke-security-group-ingress --group-id "${sg_id}" \
      --security-group-rule-ids $(aws --region "${REGION}" ec2 describe-security-group-rules \
        --filters "Name=group-id,Values=${sg_id}" --query 'SecurityGroupRules[?!IsEgress].SecurityGroupRuleId' --output text 2>/dev/null) 2>/dev/null || true
